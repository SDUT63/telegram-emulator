...
def require_migrations(db_url: str | None = None) -> None:
    ...
    Production callers must not silently skip this check.  The explicit
    SDUT_SKIP_MIGRATION_GUARD=1 escape hatch is intended only for local/unit
    test environments and is rejected by production entrypoints.
    """
    if (os.getenv("SDUT_SKIP_MIGRATION_GUARD") or "").strip() == "1":
        return
    ...
    try:
        ...
        raise RuntimeError(
            "PostgreSQL migration metadata is missing; run "
            "scripts/migrate_postgres.py first"
        ) from exc
    ...
    for version in sorted(REQUIRED_MIGRATIONS):
        ...
        if actual is None:
            raise RuntimeError(
                f"Migration {version} has no recorded SHA-256; run "
                "scripts/migrate_postgres.py before production start"
            )
        if actual != expected:
            raise RuntimeError(
