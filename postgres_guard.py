...
REQUIRED_MIGRATIONS = {
    ...
    "010_deleted_event_tombstones.sql", "011_outbox_farewell.sql", "012_outbox_attachments.sql",
}
...
def require_migrations(db_url: str | None = None) -> None:
    """Verify the deployed schema matches the committed migration set."""
    ...
    try:
        with psycopg.connect(dsn) as conn:
            applied = {str(row[0]) for row in conn.execute("SELECT version FROM schema_migrations").fetchall()}
            missing = sorted(REQUIRED_MIGRATIONS - applied)
            if missing:
                raise RuntimeError("PostgreSQL schema is not ready; apply migrations first: " + ", ".join(missing))
            checksums = {
                str(row[0]): str(row[1])
        ...
        raise RuntimeError("PostgreSQL migration metadata is missing; run scripts/migrate_postgres.py first") from exc
    ...
    for version in sorted(REQUIRED_MIGRATIONS):
        ...
        if actual is None:
            raise RuntimeError(f"Migration {version} has no recorded SHA-256; run scripts/migrate_postgres.py before production start")
        if actual != expected:
            raise RuntimeError(f"Migration {version} was modified after application: database={actual}, file={expected}. Create a new migration instead.")
