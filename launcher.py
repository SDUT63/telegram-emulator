...
def preflight() -> bool:
    """Check bot-owned files before starting a local process."""
    required = (
        "max_bot.py", "chatbot_survey.py", "survey_questions.py", "knowledge.py",
        "consent_forms.py", "fallback.py", "scale_731.py",
    )
...
def start(mode: str) -> int:
    targets = {
        ...
        "prod": ("max_webhook.py", "MAX bot / PostgreSQL production webhook"),
...
def normalize_mode(value: str | None) -> str:
    aliases = {
        None: "bot", "": "bot", "bot": "bot", "бот": "bot", "pilot": "bot",
        "prod": "prod", "production": "prod", "прод": "prod", "продакшен": "prod",
        "crm": "crm", "doctor": "doctor", "check": "check", "проверка": "check",
    }
...
def main(arguments: list[str] | None = None) -> int:
    ...
    if env_result != 0:
        return env_result
    if mode == "prod":
        if os.getenv("SDUT_ENV", "").strip().lower() != "production":
            say("Для production запуска установите SDUT_ENV=production.")
            return 7
        return start(mode)
    if not preflight():
        return 5
