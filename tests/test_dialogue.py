...
# ----------------------------------- «записал, передам» — один раз за разговор

def test_подтверждение_приходит_один_раз(consented):
    """Повторять «записал» на каждое сообщение — превратиться в автоответчик."""
    s = consented
    дозаполнить(s)
...
def test_молчание_не_теряет_сообщений(consented):
    """Бот молчит, но карточка пополняется — иначе это не молчание, а потеря."""
    s = consented
    дозаполнить(s)
    было = len(s.messages("u1"))
    for текст in ("первое сообщение координатору", "второе сообщение координатору",
                  "третье сообщение координатору"):
        s.handle("u1", текст)
    assert len(s.messages("u1")) == было + 3


@pytest.mark.parametrize("зов", [
    "перезвоните пожалуйста", "нужен оператор", "срочно помогите",
    "свяжитесь со мной сегодня",
])
def test_когда_зовут_человека_подтверждаем_всегда(consented, зов):
    """Молчание в ответ на зов читается как «меня не услышали»."""
    s = consented
    дозаполнить(s)
    s.handle("u1", "обычное сообщение без просьбы о звонке")
    assert s.handle("u1", зов) == ANSWERED


def test_через_сутки_подтверждаем_снова(consented):
    """Новый день — новый разговор, и первое сообщение в нём подтверждаем."""
