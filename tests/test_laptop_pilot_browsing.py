...
from max_ui import branch_screen, map_screen
...
def test_листание_карты_переписывает_один_экран(пилот):
    бот = Бот()
    нажатия = [
        map_screen(пилот, "5674119"),
        branch_screen("palliativ", 1, пилот, "5674119"),
        branch_screen("palliativ", 2, пилот, "5674119"),
        map_screen(пилот, "5674119"),
    ]
    правок = 0
...
def test_если_экран_переписать_нельзя_отправляем_новым(пилот):
    бот = Бот()
    asyncio.run(pilot.browse(СобытиеБезЭкрана(бот), 777, "5674119", map_screen(пилот, "5674119"), пилот))
...
def test_битая_кнопка_ведёт_на_карту_а_не_в_тишину(пилот):
    бот = Бот()
    событие = Событие(бот)
    asyncio.run(pilot.browse(событие, 777, "5674119", branch_screen("нет-такой-ветви", 1, пилот, "5674119"), пилот, "ветвь не найдена"))
