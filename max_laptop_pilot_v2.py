...
def markup(rows_):
    ...
    for row in rows_: kb.row(*[CallbackButton(text=a,payload=b,intent=Intent.POSITIVE if b in {"c:y","n"} else Intent.DEFAULT) for a,b in row])
    return kb.as_markup()

async def send(bot,chat,user,text,rows_=None):
    await bot.send_message(**({"chat_id":chat,"text":text,"attachments":[markup(rows_)] if rows_ else None} if chat is not None else {"user_id":int(user),"text":text,"attachments":[markup(rows_)] if rows_ else None}))
...
def build_dispatcher(s):
    ...
    @dp.bot_started()
    async def started(e):
        c,u=e.get_ids();u=str(u);await send(e.bot,c,u,s.start(u),rows(s,u))
    @dp.message_created()
    async def message(e):
