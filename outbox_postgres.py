...
class PostgresOutbox:
    ...
    def enqueue(self, *, delivery_key: str, user_id: str, payload: dict[str, Any], chat_id: str | None = None, conn=None) -> int:
        ...
        try:
            ...
            row = connection.execute("""
                ...
                VALUES(%s,%s,%s,%s,%s) ON CONFLICT(delivery_key) DO NOTHING RETURNING id
                """, (key, uid, chat_id, Jsonb(payload), digest)).fetchone()
            if row:
    ...
    def claim(self, *, limit: int = DEFAULT_BATCH_SIZE) -> list[OutboxMessage]:
        ...
        with _connect(self.db_url) as conn:
            stale = conn.execute("""
                SELECT id,payload,payload_sha256,attempts FROM outbox_messages
                WHERE status='sending' AND locked_at < CURRENT_TIMESTAMP - (%s * INTERVAL '1 second')
                FOR UPDATE SKIP LOCKED
                """, (self.lease_seconds,)).fetchall()
            for row in stale:
                attempts = int(row["attempts"])
                if attempts >= self.max_attempts:
                    digest = str(row["payload_sha256"] or payload_sha256(dict(row["payload"])))
                    conn.execute("""
                        UPDATE outbox_messages SET status='dead', payload=%s, payload_sha256=%s,
                        user_id=NULL, chat_id=NULL, locked_at=NULL, locked_by=NULL,
                        last_error=COALESCE(last_error, 'worker lease expired') WHERE id=%s AND status='sending'
            ...
            rows = conn.execute("""
                ...
                """, (limit, now, self.worker_id)).fetchall()
    ...
    def mark_sent(self, message_id: int) -> None:
        with _connect(self.db_url) as conn:
            ...
            updated = conn.execute("""
                UPDATE outbox_messages SET status='sent', sent_at=CURRENT_TIMESTAMP,
                locked_at=NULL, locked_by=NULL, last_error=NULL, user_id=NULL, chat_id=NULL,
                payload=%s, payload_sha256=%s WHERE id=%s AND status='sending' AND locked_by=%s
                """, (Jsonb(_REDACTED_PAYLOAD), digest, message_id, self.worker_id)).rowcount
            if updated != 1: raise RuntimeError(f"outbox message {message_id} is not owned by this worker")
    ...
    def mark_failed(self, message_id: int, error: BaseException | str) -> None:
        ...
        with _connect(self.db_url) as conn:
            ...
            if attempts >= self.max_attempts:
                ...
                conn.execute("""
                    UPDATE outbox_messages SET status='dead', payload=%s, payload_sha256=%s,
                    last_error=%s, user_id=NULL, chat_id=NULL, locked_at=NULL, locked_by=NULL
                    WHERE id=%s AND status='sending' AND locked_by=%s
                    """, (Jsonb(_REDACTED_PAYLOAD), digest, safe_error, message_id, self.worker_id))
            else:
                delay = min(3600, 2 ** min(attempts, 10))
                conn.execute("UPDATE outbox_messages SET status='pending',available_at=CURRENT_TIMESTAMP + (%s * INTERVAL '1 second'),last_error=%s,locked_at=NULL,locked_by=NULL WHERE id=%s AND status='sending' AND locked_by=%s", (delay, safe_error, message_id, self.worker_id))
    ...
    def prune_sent(self, *, retention_seconds: int = DEFAULT_SENT_RETENTION_SECONDS, limit: int = 500) -> int:
        ...
        with _connect(self.db_url) as conn:
            rows = conn.execute("""
                SELECT id,delivery_key,payload_sha256 FROM outbox_messages
                WHERE status='sent' AND sent_at < CURRENT_TIMESTAMP - (%s * INTERVAL '1 second')
                ORDER BY id LIMIT %s FOR UPDATE SKIP LOCKED
                """, (retention_seconds, limit)).fetchall()
            for row in rows:
    ...
    def _row(row: dict[str, Any]) -> OutboxMessage:
        ...
        return OutboxMessage(
            id=int(row["id"]), delivery_key=str(row["delivery_key"]),
            ...
            available_at=row["available_at"], last_error=str(row["last_error"]) if row["last_error"] is not None else None,
        )
