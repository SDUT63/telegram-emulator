...
class PostgresOutbox:
    def __init__(self, db_url: str | None = None, *, lease_seconds: int = DEFAULT_LEASE_SECONDS, max_attempts: int = DEFAULT_MAX_ATTEMPTS, worker_id: str | None = None) -> None:
        ...
        self.db_url = db_url or database_url(); self.lease_seconds = lease_seconds; self.max_attempts = max_attempts
        self.worker_id = worker_id or f"{socket.gethostname()}:{os.getpid()}:{uuid.uuid4().hex[:8]}"

    def _connect(self):
        """Compatibility/testing connection factory bound to this queue DSN."""
        return _connect(self.db_url)
        ...
        with _connect(self.db_url) as conn:
            ...
            for row in stale:
                ...
                if attempts >= self.max_attempts:
                    ...
                    conn.execute("UPDATE outbox_messages SET status='dead', payload=%s, payload_sha256=%s, user_id=NULL, chat_id=NULL, locked_at=NULL, locked_by=NULL, last_error=COALESCE(last_error, 'worker lease expired') WHERE id=%s AND status='sending'", (Jsonb(_REDACTED_PAYLOAD), digest, int(row["id"])))
    ...
    def mark_sent(self, message_id: int) -> None:
        with _connect(self.db_url) as conn:
            ...
            digest = str(row["payload_sha256"] or payload_sha256(dict(row["payload"])))
            updated = conn.execute("UPDATE outbox_messages SET status='sent', sent_at=CURRENT_TIMESTAMP, locked_at=NULL, locked_by=NULL, last_error=NULL, user_id=NULL, chat_id=NULL, payload=%s, payload_sha256=%s WHERE id=%s AND status='sending' AND locked_by=%s", (Jsonb(_REDACTED_PAYLOAD), digest, message_id, self.worker_id)).rowcount
    ...
    def mark_failed(self, message_id: int, error: BaseException | str) -> None:
        ...
        with _connect(self.db_url) as conn:
            ...
            if attempts >= self.max_attempts:
                digest = str(row["payload_sha256"] or payload_sha256(dict(row["payload"])))
                conn.execute("UPDATE outbox_messages SET status='dead', payload=%s, payload_sha256=%s, last_error=%s, user_id=NULL, chat_id=NULL, locked_at=NULL, locked_by=NULL WHERE id=%s AND status='sending' AND locked_by=%s", (Jsonb(_REDACTED_PAYLOAD), digest, safe_error, message_id, self.worker_id))
            else:
    ...
    def prune_sent(self, *, retention_seconds: int = DEFAULT_SENT_RETENTION_SECONDS, limit: int = 500) -> int:
        ...
        with _connect(self.db_url) as conn:
            ...
            for row in rows:
                ...
                conn.execute("INSERT INTO outbox_delivery_tombstones(delivery_key,payload_sha256) VALUES(%s,%s) ON CONFLICT(delivery_key) DO UPDATE SET payload_sha256=EXCLUDED.payload_sha256", (row["delivery_key"], digest))
