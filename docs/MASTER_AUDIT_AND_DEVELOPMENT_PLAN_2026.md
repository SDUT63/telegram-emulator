...
6. **GitHub Actions ни разу не запускался** на этой ветке. Локальный
   прогон (946 passed, PostgreSQL 16) его не заменяет.
7. **Домен, TLS и вебхук на 443** за обратным прокси.
...
10. **Ограничение частоты входящих.** Защита не от нагрузки (её
    проверили: 30 человек одновременно проходят без ошибок), а от
    злонамеренного потока по публичной рекламе.
11. **Отметка актуальности статей базы знаний.** У статьи есть источник,
    но нет даты проверки: изменится приказ — никто не узнает, что статья
    устарела.
12. **Единая реализация анкеты для пилота и production.** Сейчас две
...
6. Запустить GitHub Actions на HEAD и устранить расхождения.
7. Провести восстановление из копии на отдельной базе, замерить RTO/RPO.
8. Подключить сборщик метрик и дежурство.
