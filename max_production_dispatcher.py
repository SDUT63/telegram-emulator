...
DELETION_WORDS = {"удалить данные", "удалить мои данные", "удалить анкету", "удалить мои данные и анкету"}
...
def _resolve_article_callback(token: str) -> str | None:
    ...
    if len(digest) != 16 or any(ch not in "0123456789abcdef" for ch in digest):
        return None
    import knowledge
    matches = [article.заголовок for article in knowledge.загрузить() if hashlib.sha256(article.заголовок.encode("utf-8")).hexdigest()[:16] == digest]
...
def _callback_event_id(event, callback_id: str, user_id: str) -> str | None:
    ...
    return _fingerprint_event("callback", user_id, chat_id, message_id, callback_id, payload, getattr(event, "timestamp", None))


def _started_event_id(event, user_id: str) -> str | None:
    return _fingerprint_event("bot_started", user_id, getattr(event, "chat_id", None), getattr(event, "timestamp", None))
...
def build_dispatcher(survey, seen=None):
    ...
    async def on_started(event: BotStarted) -> None:
        ...
        if event_key is None: log.error("%s: BotStarted without stable event fields rejected", uid); return
        if _deleted_replay(survey, event_key): return
        async def mutate() -> None:
            if survey.start_event(uid): log.info("%s: BotStarted transition committed to outbox", uid)
        await _with_event_id(event_key, mutate)

    @dp.message_created()
    async def on_message(event: MessageCreated) -> None:
        _, user_id = event.get_ids(); uid = str(user_id); body = getattr(getattr(event, "message", None), "body", None)
        event_key = _event_id(getattr(body, "mid", None))
        if event_key is None: log.error("%s: message without provider mid rejected", uid); return
        if _deleted_replay(survey, event_key): return
        incoming = (getattr(body, "text", None) or "").strip(); files = files_of(body)
        async def mutate() -> None:
            normalized = incoming.lower().strip(" ?!.")
            if normalized in DELETION_WORDS:
                survey.delete_user(uid)
                log.info("%s: user data purge committed", uid)
                return
    ...
    async def on_button(event: MessageCallback) -> None:
        _, user_id = event.get_ids(); uid = str(user_id); callback = event.callback
        callback_id = _event_id(getattr(callback, "callback_id", None))
        if callback_id is None: log.error("%s: callback without button callback_id rejected", uid); return
        event_key = _callback_event_id(event, callback_id, uid)
        if event_key is None: log.error("%s: callback without stable event fields rejected", uid); return
        if _deleted_replay(survey, event_key): await acknowledge(event, "Принято"); return
        payload = (getattr(callback, "payload", None) or "")
        if len(payload) > 512: log.warning("%s: oversized callback payload rejected", uid); return
        async def mutate() -> None:
            ...
            if action == "k" and payload.startswith("k:"):
                ...
                if title is None: log.warning("%s: invalid or ambiguous knowledge callback rejected", uid); return
