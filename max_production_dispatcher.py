...
Production entrypoints use this dispatcher. It never calls the MAX network
API directly: inbound events mutate PostgreSQL and create durable outbound
intents; the worker owns actual delivery.
"""
...
def build_dispatcher(survey, seen):
    """Build the production dispatcher around durable event boundaries."""
    ...
    async def on_started(event: BotStarted) -> None:
        """Start/resume a case and queue the first durable reply."""
        _, user_id = event.get_ids()
        uid = str(user_id)
        event_key = getattr(event, "event_id", None) or getattr(event, "id", None)
        if not seen.fresh(event_key):
            log.info("%s: duplicate BotStarted ignored", uid)
            return
        reply = survey.start_event(uid)
        if reply:
            log.info("%s: BotStarted reply committed to outbox", uid)

    @dp.message_created()
    async def on_message(event: MessageCreated) -> None:
        """Process one MAX message exactly once at the PostgreSQL boundary."""
        _, user_id = event.get_ids()
        uid = str(user_id)
        body = event.message.body
        event_key = getattr(body, "mid", None)
        if not seen.fresh(event_key):
            log.info("%s: duplicate message ignored", uid)
            return

        incoming = ((body.text if body else None) or "").strip()
        files = []
        from max_bot import _files_of, ASK_WORDS
    ...
    async def on_button(event: MessageCallback) -> None:
        """Apply callback state and create its reply in the same transaction."""
        _, user_id = event.get_ids()
        uid = str(user_id)
        callback = event.callback
        callback_id = getattr(callback, "callback_id", None)
        if not seen.fresh(callback_id):
            log.info("%s: duplicate callback ignored", uid)
            return
        ...
        if action == "k" and args:
            survey.handle_navigation_event(uid, "k", [payload[2:]])
            return
        if action == "q":
