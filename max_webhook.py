...
def validate_secret(secret: str) -> list[str]:
    ...
    if not SECRET_RE.fullmatch(secret): return ["MAX_WEBHOOK_SECRET должен содержать 5–256 символов: A-Z, a-z, 0-9, _ или -"]
    return []
...
def validate_public_url(url: str, expected_path: str) -> list[str]:
    ...
    path = parsed.path or "/"
    if expected_path and path != expected_path: problems.append(f"путь URL ({path}) должен совпадать с MAX_WEBHOOK_PATH ({expected_path})")
    return problems
...
async def main() -> None:
    ...
    try:
        ...
        except Exception as error: log.warning("Меню команд не обновилось: %s", error)
        ...
        print("\n" + "=" * 70); print(f"  Бот запущен через MAX Webhook: {name}"); print(f"  Внешний endpoint: {public_url}"); print(f"  Внутренний listener: {host}:{port}{path}"); print("  Хранилище: PostgreSQL"); print("  Outbound: durable PostgreSQL outbox"); print("  Privacy: transactional user purge"); print("=" * 70 + "\n")
        runner = web.AppRunner(app); await runner.setup(); site = web.TCPSite(runner, host=host, port=port); await site.start()
