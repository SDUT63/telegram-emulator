...
_TX_CONNECTION: contextvars.ContextVar[Any | None] = contextvars.ContextVar("sdut_tx_connection", default=None)
_TX_USER: contextvars.ContextVar[str | None] = contextvars.ContextVar("sdut_tx_user", default=None)
_TX_EVENT: contextvars.ContextVar[str | None] = contextvars.ContextVar("sdut_tx_event", default=None)
_TX_ACCEPTED: contextvars.ContextVar[bool] = contextvars.ContextVar("sdut_tx_accepted", default=False)


def database_url() -> str:
    value = (os.getenv("SDUT_DATABASE_URL") or "").strip()
    if not value:
        raise RuntimeError("SDUT_DATABASE_URL не задан. Для PostgreSQL укажите postgresql://user:password@host:5432/database")
    return value
...
class PostgresSurvey(Survey):
    ...
    def audit(self, user_id: str | None, event_type: str, payload: dict[str, Any]) -> None:
        ...
        if active is not None:
            active.execute("INSERT INTO audit_events(user_id,event_type,event_json) VALUES(%s,%s,%s)", (user_id, event_type, Jsonb(payload)))
            return
        with self._db_lock, self._connect() as conn:
            conn.execute("INSERT INTO audit_events(user_id,event_type,event_json) VALUES(%s,%s,%s)", (user_id, event_type, Jsonb(payload)))
...
class TransactionalPostgresSurvey(PostgresSurvey):
    ...
    def _begin_event(self, user_id: str, event_type: str, payload: dict[str, Any] | None = None):
        ...
        try:
            ...
            row = conn.execute("SELECT state_json FROM survey_state WHERE user_id=%s FOR UPDATE", (str(user_id),)).fetchone()
    ...
    def handle(self, user_id: str, text: str) -> str:
        return self._mutate(user_id, "message", {"kind": "message"}, lambda: Survey.handle(self, user_id, text), "")

    def grant_consent(self, user_id: str) -> str:
        return self._mutate(user_id, "callback", {"action": "grant_consent"}, lambda: Survey.grant_consent(self, user_id), "")

    def refuse_consent(self, user_id: str) -> str:
        return self._mutate(user_id, "callback", {"action": "refuse_consent"}, lambda: Survey.refuse_consent(self, user_id), "")

    def toggle(self, user_id: str, step: int, index: int) -> bool:
        return self._mutate(user_id, "callback", {"action": "toggle", "step": step, "index": index}, lambda: Survey.toggle(self, user_id, step, index), False)

    def answer_by_numbers(self, user_id: str, numbers: list[int]) -> str:
        return self._mutate(user_id, "callback", {"action": "answer", "numbers": numbers}, lambda: Survey.answer_by_numbers(self, user_id, numbers), "")
