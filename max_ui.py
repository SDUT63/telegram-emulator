...
This module deliberately has no maxapi dependency. It produces text and
serialized keyboard rows; the only network boundary is the outbound
transport adapter.
"""
...
КАРТА_ПОДПИСЬ = (
    "Выберите, что ближе. Или просто напишите вопрос "
...
def layout(survey: Survey, user_id: str) -> list[list[tuple[str, str]]]:
    """Return the canonical questionnaire keyboard as label/payload pairs."""
    ...
    for index in shown:
        if multi:
            label = fits((MARK_ON if index in picked else MARK_OFF) + options[index])
            action = f"t:{step}:{index}"
        else:
            label = fits(options[index])
            action = f"a:{step}:{index}"
...
def navigation_keyboard(action: str, args: list[str], survey: Survey, user_id: str) -> list[list[list[str]]]:
    """Serialize knowledge navigation into MAX-independent keyboard rows."""
    rows: list[list[list[str]]] = []
    if action == "map":
        for branch in knowledge.карта():
            rows.append([[fits(branch["название"]), "v:" + branch["id"] + ":1"]])
        if _in_questionnaire(survey, user_id):
    ...
    if action == "v" and args:
        ...
        for title in page["статьи"]:
            rows.append([[fits(knowledge.подпись(title)), "k:" + title[:60]]])
        nav: list[list[str]] = []
        if page["номер"] > 1:
    ...
    if action == "k" and args:
        ...
        for neighbor in knowledge.соседи(title, сколько=3):
            rows.append([[fits(knowledge.подпись(neighbor)), "k:" + neighbor[:60]]])
    ...
    if action == "q":
        return [[[label, payload] for label, payload in row] for row in layout(survey, user_id)]
    return rows
...
def map_screen(survey: Survey | None = None, user_id: str = "") -> tuple[str, list[list[list[str]]]]:
    return (KАРТА_ЗАГОЛОВОК + "\n\n" + KАРТА_ПОДПИСЬ,
            navigation_keyboard("map", [], survey, user_id) if survey is not None else navigation_keyboard("map", [], Survey.__new__(Survey), user_id))


def branch_screen(branch_id: str, page_number: int = 1, survey: Survey | None = None,
                  user_id: str = "") -> tuple[str, list[list[list[str]]]] | None:
    page = knowledge.страница(branch_id, page_number)
    if not page:
        return None
    last = page["первая"] + len(page["статьи"]) - 1
    header = page["название"]
    if page["всего"] > 1:
        header += f"\n\nТемы {page['первая']}–{last} из {page['статей']}"
    return header, navigation_keyboard("v", [branch_id, str(page_number)], survey or Survey.__new__(Survey), user_id)
...
def article_screen(title: str, survey: Survey | None = None, user_id: str = "") -> tuple[str, list[list[list[str]]]] | None:
    ...
    if breadcrumbs:
        text = breadcrumbs.rstrip(" ·") + "\n\n" + text
    return text + "\n\n" + СПРАВКА_ПОДПИСЬ, navigation_keyboard("k", [title], survey or Survey.__new__(Survey), user_id)


def files_of(body: Any) -> list[dict[str, Any]]:
    """Extract attachment metadata without downloading file contents."""
