...
# Подпись под выдержкой из материалов службы. Держим здесь, а не
# в max_ui: справку отдают оба транспорта, а max_ui знает про кнопки MAX.
СПРАВКА_ПОДПИСЬ = (
    "———\nЭто выдержка из материалов службы. Координатор ответит "
    "подробнее при звонке, а если человеку плохо сейчас — звоните 103."
)
...
class Survey:
    ...
    def handle(self, user_id: str, text: str) -> str:
        ...
        if step >= len(QUESTIONS) or person["finished"]:
            ...
            # Человек зовёт живого человека — «перезвоните», «нужен
            # оператор». Это просьба о связи, а не вопрос: подтверждение
            # тут и есть ответ по существу, а статья из материалов
            # службы была бы отпиской вместо обещанного звонка.
            зовут = any(с in low for с in ЗОВУТ_ЧЕЛОВЕКА)
            подтвердить = self._надо_подтвердить(user_id, low)
            if зовут:
                return ANSWERED if подтвердить else ""

            # Иначе сначала ответ по существу, и только потом обещание
            # передать координатору. Раньше подтверждение возвращалось
            # вместо справки, и первый же вопрос после анкеты человек
            # задавал впустую: «пролежни» → «записал, передам».
            справка = self.справка_по_вопросу(user_id, text)
            if подтвердить:
                return справка + "\n\n" + "—" * 20 + "\n\n" + ANSWERED
            return справка
    ...
    def continue_detailed(self, user_id: str) -> str:
        """Вернуться к подробной части после «Достаточно, свяжитесь».

        Имя, телефон и адрес уже собраны — их не спрашиваем заново:
        человек согласился потратить ещё пять минут, а не проходить
        всё сначала.

        Жил в пилоте, перенесён сюда: кнопка «продолжить подробную
        анкету» есть у обоих транспортов, а делать её было нечем
        только у одного.
        """
        uid = str(user_id)
        человек = self.state.get(uid)
        if not человек or not человек.get("finished"):
            return self.question_text(uid)

        рубеж = next(i for i, q in enumerate(QUESTIONS) if q["id"] == CHECKPOINT_ID)
        оставить = {q["id"] for q in QUESTIONS[:рубеж + 1]} | {"district", "lift"}
        ответы = {к: v for к, v in (человек.get("answers") or {}).items() if к in оставить}
        ответы[CHECKPOINT_ID] = "Продолжить"

        человек["answers"] = ответы
    ...
    def есть_что_продолжить(self, user_id: str) -> bool:
        ...
        «Достаточно, свяжитесь» тоже помечает анкету законченной, но
        подробная часть при этом не пройдена — её можно продолжить,
        и предложить это правильно. А тому, кто ответил на всё,
        кнопка «продолжить подробную анкету» врёт: продолжать нечего.
        """
        человек = self.state.get(str(user_id)) or {}
        if not человек.get("finished"):
            return False
        return (человек.get("answers") or {}).get(CHECKPOINT_ID) == STOP_OPTION

    def справка_по_вопросу(self, user_id: str, вопрос: str) -> str:
        """Ответ из материалов службы на свободный вопрос.

        Живёт здесь, а не в диспетчере, потому что нужен обоим: и боевому,
        и ноутбучному. Пока он был только в боевом, пилот на вопрос после
        анкеты не отвечал вовсе — `handle` возвращал пустую строку,
        а пустую строку пилот не отправляет. Человек писал «пролежни»
        и не получал ничего.

        Не нашлось — отвечаем лесенкой из fallback: молчание здесь
        недопустимо ни при каком исходе.
        """
        import knowledge

        текст = knowledge.ответ_без_модели(вопрос or "")
        if текст:
            self.understood(user_id)
            return текст + "\n\n" + СПРАВКА_ПОДПИСЬ
