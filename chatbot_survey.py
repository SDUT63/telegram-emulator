...
# Ответ принят не кнопкой, а словами человека. Сказать об этом надо:
# иначе он не знает, засчиталось ли, и пишет то же самое ещё раз.
ЗАПИСАНО_СЛОВАМИ = (
    "Записал вашими словами: «{текст}». Координатор прочитает как есть.\n\n"
    + "—" * 20
)

# Предел для ответа, записанного своими словами. Ответ на вопрос анкеты —
# это фраза, а не рассказ: триста знаков хватает на любой настоящий ответ
# с запасом. Предел нужен не из скупости — без него в карточку уходит
# столько, сколько человек прислал, ответ бота перерастает предел MAX
# в 32 000 знаков и не доходит вовсе, а публичный бот превращается
# в способ раздуть базу одним сообщением.
ПРЕДЕЛ_СВОИХ_СЛОВ = 300

# С какой длины написанное перестаёт быть ответом на вопрос и становится
# рассказом о том, что дома. «Мама» — ответ. «Мама после инсульта, лежит
# второй год, я одна с ней» — рассказ, и его надо сохранить целиком,
# даже если в карточку по нему пошла подпись кнопки.
РАССКАЗ_ОТ = 120

# Длинный текст при этом не пропадает: он не ответ на вопрос с кнопками,
# а рассказ о том, что дома, — и координатору он нужнее короткого.
ЗАПИСАНО_ДЛИННОЕ = (
    "Записал. В ответ на вопрос пошло начало, а всё написанное целиком "
    "передам координатору — он прочитает.\n\n" + "—" * 20
)
...
# Сколько часов подряд считаются одним разговором. Внутри разговора
# «записал, передам» говорится один раз: повторять его на каждое сообщение
# значит превратить живой ответ в автоответчик — человек перестаёт читать
# и перестаёт писать. Всё написанное попадает в карточку в любом случае,
# подтверждение об этом только сообщает.
СЕССИЯ_ЧАСОВ = 12

# Слова, при которых подтверждаем всегда, сколько бы раз за разговор
# это ни случилось: человек зовёт живого человека или говорит о срочном,
# и молчание здесь читается как «меня не услышали».
ЗОВУТ_ЧЕЛОВЕКА = (
...
class Survey:
    ...
    def _надо_подтвердить(self, user_id: str, low: str) -> bool:
        """Говорить ли «записал, передам» на это сообщение.

        Один раз за разговор — и всегда, когда человек зовёт живого
        человека. В остальных случаях подтверждение только шумит.
        """
        person = self.state.get(user_id)
        if person is None:
            return True

        зовут = any(с in low for с in ЗОВУТ_ЧЕЛОВЕКА)
        если_было = person.get("acked")
        свежо = False
        if если_было and not зовут:
            try:
                прошло = datetime.now() - datetime.fromisoformat(если_было)
                свежо = прошло.total_seconds() < СЕССИЯ_ЧАСОВ * 3600
            except (TypeError, ValueError):
                свежо = False
        if свежо:
            return False
    ...
    def _тревога(self, user_id: str, text: str) -> str | None:
        ...
        # он отвечает, а не зовёт на помощь, — и ответ надо записать.
    ...
    def handle(self, user_id: str, text: str) -> str:
        ...
        if step >= len(QUESTIONS) or person["finished"]:
            ...
            if self._надо_подтвердить(user_id, low):
                return ANSWERED
            # Молчим намеренно: сообщение записано, а ответ по существу
            # придёт справкой из базы знаний следом.
            return ""
        ...
        if not ok:
            ...
            # На второй ступени бот обещает: «напишите словами, как есть
            # на самом деле, координатор разберётся, ответ не пропадёт».
            # Обещание должно быть настоящим. Раньше следующая же фраза
            # человека отвергалась снова — и человек, пришедший из
            # социальной рекламы, упирался в один вопрос и уходил.
            if попытка > 1 and self._свои_слова(question, text):
                # Длинное не обрезается молча: человек должен знать,
                # что в карточку пошло, а что — в переписку.
                if len(text) > ПРЕДЕЛ_СВОИХ_СЛОВ:
                    if not self._looks_like_speech(text):
                        self.note_message(user_id, text)
                    коротко = text[:ПРЕДЕЛ_СВОИХ_СЛОВ].rstrip() + "…"
                    return ЗАПИСАНО_ДЛИННОЕ + "\n\n" + self._accept(
                        user_id, step, коротко)
                return (
                    ЗАПИСАНО_СЛОВАМИ.format(текст=text)
                    + "\n\n" + self._accept(user_id, step, text)
                )
            подсказка = ""
        ...
        # Ответ разобран — но человек мог написать куда больше, чем
        # подпись кнопки: «мама после инсульта, лежит второй год, я одна
        # с ней». В карточку по такой фразе уходит «О близком человеке»,
        # а сам рассказ прежде выбрасывался. Он координатору нужнее
        # ответа: с него начинается разговор.
        if len(text) >= РАССКАЗ_ОТ and text != cleaned:
            self.note_message(user_id, text)
    ...
    def _свои_слова(question: dict[str, Any], text: str) -> bool:
        """Можно ли принять это как ответ, записанный словами человека.

        Принимаем там, где вариантов ответа заведомо не хватает на всю
        жизнь: кнопки. Телефон, адрес и почта проверяются по существу —
        там «как есть» не работает: по слову вместо номера координатор
        не дозвонится, и обещать обратное нельзя.

        От самого текста нужно, чтобы он был похож на речь. «7», «...»,
        «ыыы» — промах по кнопке или стук по клавиатуре, и записывать их
        в карточку как ответ значит соврать координатору. Признак
        простой и без словаря: в настоящем слове есть и гласная,
        и согласная.
        """
        if question.get("kind") != "choice":
            return False

        буквы = re.sub(r"[^а-яёa-z]", "", (text or "").lower())
        if len(буквы) < 3:
            return False
        if len(set(буквы)) < 2:                       # «ыыы», «ааааа»
            return False

        гласные = set("аеёиоуыэюяaeiouy")
        есть_гласная = any(б in гласные for б in буквы)
        есть_согласная = any(б not in гласные for б in буквы)
        return есть_гласная and есть_согласная
    ...
    def _check(self, question: dict[str, Any], text: str) -> tuple[bool, str, str]:
        ...
        if kind == "phone":
            ...
            # Верхняя граница не меньше важна нижней: по номеру перезванивают.
            # Российский — 11 цифр, международный с кодом страны укладывается
            # в 15 (E.164). Всё, что длиннее, — это опечатка или два номера
            # подряд, и координатор по такому не дозвонится.
            if len(digits) > 15:
                return False, "", (
                    "В номере слишком много цифр. Если номеров два — "
                    "напишите один, по которому удобнее дозвониться."
                )
            return True, text, ""
    ...
    @staticmethod
    def _check_one(question: dict[str, Any], text: str) -> tuple[bool, str, str]:
        options: list[str] = question["options"]
        ид = question.get("id", "")

        # Возраст числом — не промах по кнопке, а ответ. Проверяем это
        # раньше номеров: «82» в вопросе о возрасте значит восемьдесят
        # два года, а не восемьдесят второй вариант.
        точно = survey_questions.толкование(ид, text)
        if точно:
            return True, точно, ""
        ...
        узнанное = Survey._по_синонимам(ид, options, text)
        if узнанное:
            return True, узнанное, ""

        return False, "", f"Напишите номер ответа — от 1 до {len(options)}."

    @staticmethod
    def _по_синонимам(ид: str, options: list[str], text: str) -> str | None:
        """Вариант, на который человек указал своими словами.

        Возвращает вариант, только если указание однозначно. «Нужна
        сиделка для мамы» указывает и на «Я специалист», и на «О близком
        человеке» — тогда не возвращает ничего, и ответ уйдёт словами
    ...
    def _попадания(ид: str, options: list[str], text: str) -> dict[str, str]:
        ...
        слова_строки = низом.split()

        def указывает(основа: str) -> bool:
            основа = основа.replace("ё", "е")
            if " " in основа:            # устойчивое сочетание — ищем целиком
                return основа in низом
            # «=да» — слово целиком и только оно. Без этого «да» поймало
            # бы «давление», а «нет» — «нету сил». Помечено в словаре
            # явно: по длине такое не угадывается — «мам» тоже коротка,
            # но ей нужны окончания.
            if основа.startswith("="):
                return основа[1:] in слова_строки
            # Обычная основа сравнивается с началом слова: «мам» ловит
            # «маме» и «мамой», не встречаясь посреди чужого слова.
            return any(слово.startswith(основа) for слово in слова_строки)

        подошли: dict[str, str] = {}
        for имя, основы in словарь.items():
            if имя not in options:
                continue
            попавшие = [о.replace("ё", "е").lstrip("=")
                        for о in основы if указывает(о)]
            if попавшие:
                подошли[имя] = max(попавшие, key=len)
