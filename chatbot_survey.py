...
GREETING = (
    ...
    "Мы помогаем семьям, которые ухаживают дома за пожилым или тяжелобольным человеком.\n\n"
    "Задам несколько вопросов — это минут пять — и передам всё координатору, живому человеку, который вам перезвонит.\n\n"
...
HELP = (
    ...
    "отмена — прервать\n\n"
    "Анкета сохраняется. Можно закрыть и вернуться позже — продолжим с того же места."
)

DONE_FULL = "Спасибо, анкета заполнена. Координатор свяжется с вами.\n\nЕсли состояние ухудшится до того, как мы позвоним, — звоните 103."
DONE_SHORT = "Спасибо, записали. Координатор свяжется с вами по указанному телефону.\n\nЕсли захотите дополнить — нажмите «Продолжить анкету»: имя, телефон и адрес уже сохранены.\nЕсли станет хуже — звоните 103."
CONSENT_VERSION = "1.0"
...
CONSENT_SHORT = "Сначала одно короткое дело.\n\nВопросы будут о здоровье — как человек ходит, ест, спит, что беспокоит. Такие сведения закон разрешает записывать только с вашего согласия.\n\nКоротко, о чём речь:\n\n• записываем имя, телефон, адрес и ваши ответы;\n• читают их только координаторы службы;\n• врачам передаём лишь с вашего отдельного разрешения;\n• напишете «удалить» — сотрём всё и подтвердим.\n\nПолный текст — по кнопке ниже, прямо здесь в чате."
CONSENT_FULL = ("Согласие на обработку персональных данных\n\nКТО СОБИРАЕТ\n" f"{ORG_FULL}\n" f"ОГРН {ORG_OGRN}, ИНН {ORG_INN}\n" f"{ORG_ADDRESS}\n\n"
    "ЧТО ЗАПИСЫВАЕМ\nИмя, по которому к вам обращаться, и телефон для связи. Имя человека, которому нужна помощь, и адрес, куда приезжать. Ваши ответы о его состоянии: как он ходит, ест, спит, что беспокоит, кто рядом. Дату обращения.\n\n"
    "Ответы о состоянии здоровья — особая категория данных, часть 1 статьи 10 закона 152-ФЗ. Без вашего согласия мы их не записываем.\n\n"
    "ЗАЧЕМ\nЧтобы координатор перезвонил, понял, что нужно, и подобрал помощь: уход на дому, обучение родных, оборудование, оформление документов.\n\n"
    "ЧТО МЫ С НИМИ ДЕЛАЕМ\nЗаписываем, храним, показываем координаторам службы и готовим по ним сводку к звонку. Ни рекламы, ни продажи, ни передачи кому-то ещё.\n\n"
    "КТО ВИДИТ\nТолько сотрудники службы, которые ведут ваше обращение. В медицинскую организацию — лишь с вашего отдельного согласия, и его вы подписываете отдельно.\n\n"
    "СКОЛЬКО ХРАНИМ\nПока вы не попросите удалить. Срока в годах нет — есть ваше слово.\n\n"
    "КАК ОТОЗВАТЬ\nНапишите боту «удалить» — в любой момент, без объяснения причин. Согласие прекращается, записи стираются, мы подтверждаем сообщением.\n\n"
    "КАК ВЫ ЕГО ДАЁТЕ\nНажатием кнопки «Согласен, продолжим». Мы записываем дату, время и редакцию этого текста. При встрече координатор даст ту же форму на бумаге — подписать её нужно будет один раз.")
CONSENT_GIVEN_AT = "Вы дали согласие {когда}. Отозвать — напишите «удалить»."
CONSENT_WAIT = "Пока кнопка не нажата, я ничего не записываю.\n\n«Согласен» — начнём анкету.\n«Полностью» — пришлю полный текст согласия сюда же.\n«Нет» — закроем, и я больше не побеспокою."
CONSENT_NO = "Понимаем, и настаивать не будем.\n\nБез согласия заполнить анкету нельзя — так требует закон. Но помощь от этого не закрывается: напишите нам в сообществах службы, там можно спросить что угодно, ничего о себе не сообщая.\n\nПередумаете — напишите «начать», и мы продолжим."
CONSENT_YES = "Спасибо, согласие записали. Теперь к делу."
ERASED = "Готово, всё удалено: и ответы, и контакты.\n\nЕсли понадобится помощь — просто напишите сюда, начнём заново."
RESUMED = "Анкета не закончена — продолжаем с того места, где остановились.\n\nЕсли хотите начать сначала, напишите «заново»."
ALREADY_DONE = "Анкета уже заполнена — координатор с вами свяжется.\n\n«ответы» — посмотреть заполненное, «заново» — пройти подробную часть ещё раз."
ANSWERED = "Записал, передам координатору — он свяжется с вами."

# Эти константы являются частью текстового интерфейса legacy SQLite-пилота.
# Их нельзя удалять при hardening production MAX: legacy launcher должен
# оставаться запускаемым и тестируемым.
СЕССИЯ_ЧАСОВ = 12
ЗОВУТ_ЧЕЛОВЕКА = (
    "оператор", "координатор", "позвоните", "перезвоните", "свяжитесь",
    "нужен человек", "живой человек", "срочно", "помогите", "скорее",
    "никто не звонит", "не дозвон", "умирает", "плохо прямо сейчас",
)
...
READ_WORDS = {"полностью", "полный текст", "согласие", "прочитать полностью", "подробнее", "/consent"}
...
# The Survey implementation below is the hardened version currently used by
# the SQLite pilot. It deliberately remains transport-independent.
...
class Survey:
    ...
    def __init__(self, storage_path: str = STORAGE, *, list_options: bool = True) -> None:
        self.storage_path = storage_path; self.list_options = list_options; self.state: dict[str, dict[str, Any]] = {}; self.load()

    def load(self) -> None:
        if not os.path.exists(self.storage_path): self.state = {}; return
        try:
            with open(self.storage_path, encoding="utf-8") as fh: self.state = json.load(fh)
        except (json.JSONDecodeError, OSError):
            try: os.replace(self.storage_path, self.storage_path + ".broken")
            except OSError: pass
            self.state = {}

    def save(self) -> None:
        tmp = self.storage_path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as fh: json.dump(self.state, fh, ensure_ascii=False, indent=2)
        os.replace(tmp, self.storage_path)

    def _person(self, user_id: str) -> dict[str, Any]:
        person = self.state.get(user_id)
        if person is None: person = self._blank(); self.state[user_id] = person
        for key, value in self._blank().items(): person.setdefault(key, value)
        return person

    @staticmethod
    def _blank() -> dict[str, Any]:
        return {"step": 0, "answers": {}, "alerts": [], "history": [], "started": None, "finished": None, "total_seen": 0, "consent": None, "pending": None, "reading": False, "messages": [], "missed": 0, "acked": None}

    @staticmethod
    def _asked(index: int, answers: dict[str, str]) -> bool:
        when = QUESTIONS[index].get("when"); return True if when is None else bool(when(answers))
    ...
    def _progress(self, person: dict[str, Any], index: int) -> str:
        answers = person["answers"]; total = sum(1 for q in QUESTIONS if q.get("when") is None or q["when"](answers)); total = max(total, person.get("total_seen", 0)); person["total_seen"] = total
        seen = sum(1 for i, q in enumerate(QUESTIONS) if i <= index and (q.get("when") is None or q["when"](answers))); return f"Вопрос {seen} из {total}"
    ...
    def consented(self, user_id: str) -> dict[str, Any] | None:
        mark = (self.state.get(user_id) or {}).get("consent") or {}; return mark if mark.get("at") else None

    def grant_consent(self, user_id: str) -> str:
        person = self._person(user_id)
        if not (person.get("consent") or {}).get("at"): person["consent"] = {"at": datetime.now().isoformat(timespec="seconds"), "version": CONSENT_VERSION}
        if person["started"] is None: person["started"] = person["consent"]["at"]
        self.save(); return CONSENT_YES + "\n\n" + self._ask(user_id, self._next(0, person["answers"]))

    def refuse_consent(self, user_id: str) -> str:
        person = self._person(user_id); person["consent"] = {"refused": datetime.now().isoformat(timespec="seconds"), "version": CONSENT_VERSION}; person["answers"] = {}; self.save(); return CONSENT_NO

    def consent_text(self, user_id: str) -> str:
        chunks = [CONSENT_FULL]; signs = consent_forms.signs((self.state.get(user_id) or {}).get("answers") or {})
    ...
    def note_message(self, user_id: str, text: str, files: list | None = None) -> None:
        ...
        record: dict[str, Any] = {"at": datetime.now().isoformat(timespec="seconds"), "text": text[:2000]}
        if files: record["files"] = [dict(f) for f in files][:10]
        journal = person.setdefault("messages", []); journal.append(record); del journal[:-self.MESSAGES_LIMIT]; self.save()

    def messages(self, user_id: str) -> list[dict[str, Any]]: return list((self.state.get(user_id) or {}).get("messages") or [])

    def _надо_подтвердить(self, user_id: str, low: str) -> bool:
        person = self.state.get(user_id)
        if person is None: return True
        зовут = any(s in low for s in ЗОВУТ_ЧЕЛОВЕКА); last = person.get("acked"); fresh = False
        if last and not зовут:
            try: fresh = (datetime.now() - datetime.fromisoformat(last)).total_seconds() < СЕССИЯ_ЧАСОВ * 3600
    ...
    def restart_after_consent(self, user_id: str) -> str:
        person = self._person(user_id); checkpoint = next(i for i, q in enumerate(QUESTIONS) if q["id"] == CHECKPOINT_ID); keep_ids = {q["id"] for q in QUESTIONS[:checkpoint + 1]}; keep_ids.update({"district", "lift"})
        answers = {key: value for key, value in person.get("answers", {}).items() if key in keep_ids}; answers[CHECKPOINT_ID] = "Продолжить"; person["answers"] = answers; person["step"] = self._next(checkpoint + 1, answers); person["history"] = [i for i in person.get("history", []) if i < person["step"]]; person["pending"] = None; person["alerts"] = []; person["finished"] = None; person["total_seen"] = 0; person["reading"] = False; self.save()
    ...
    def handle(self, user_id: str, text: str) -> str:
        ...
        if low in SUMMARY_WORDS: return self.summary(user_id)
        if low in CANCEL_WORDS: self.state.pop(user_id, None); self.save(); return "Анкета отменена. Напишите «заново», когда будете готовы."
        person = self._person(user_id); answers = person["answers"]; step = self._next(person["step"], answers)
        if self.stage(user_id) == "consent":
            if low in AGREE_WORDS or low in BEGIN_WORDS or low in RESTART_WORDS: self.understood(user_id); return self.grant_consent(user_id)
            if low in REFUSE_WORDS: self.understood(user_id); return self.refuse_consent(user_id)
            if low in READ_WORDS: return self.consent_text(user_id)
            attempt = self.miss(user_id); return CONSENT_WAIT if attempt == 1 else fallback.фраза(attempt, fallback.СОГЛАСИЕ)
        if low in RESTART_WORDS: return self.restart_after_consent(user_id)
        if step >= len(QUESTIONS) or person["finished"]:
            if low in BEGIN_WORDS: return self.start(user_id)
            self.note_message(user_id, text); return ANSWERED if self._надо_подтвердить(user_id, low) else ""
        ...
        if not ok:
            ...
            attempt = self.miss(user_id); hint = fallback.фраза(attempt, fallback.АНКЕТА, пропуск=not question.get("required", True)) + "\n\n" if attempt > 1 or question.get("options") else ""
            return problem + "\n\n" + hint + self._ask(user_id, step)
        return self._accept(user_id, step, cleaned)

    @staticmethod
    def _looks_like_speech(text: str) -> bool:
        text = (text or "").strip(); return len(re.sub(r"\D", "", text)) >= 10 or (len(text) >= 12 and " " in text)

    @staticmethod
    def _alert_rules(question: dict[str, Any]) -> list[dict[str, Any]]: return question.get("alerts") or ([question["alert"]] if question.get("alert") else [])
    @staticmethod
    def _alert_fires(rule: dict[str, Any], value: str) -> bool:
        ...
        if least and len([p for p in value.split(";") if p.strip() and "ничего" not in p.lower()]) >= least: return True
        return False

    def _accept(self, user_id: str, step: int, value: str) -> str:
        person = self._person(user_id); self.understood(user_id); question = QUESTIONS[step]; person["answers"][question["id"]] = value; person["history"].append(step); person["pending"] = None
        if question.get("derive"): person["answers"].update(question["derive"](value))
        if person["started"] is None: person["started"] = datetime.now().isoformat(timespec="seconds")
        prefix = ""
        for rule in self._alert_rules(question):
            if self._alert_fires(rule, value):
                prefix += rule["text"] + "\n\n" + "—" * 20 + "\n\n"; note = f"{rule.get('label', question['text'])}: {value}"
                if note not in person["alerts"]: person["alerts"].append(note)
        if question["id"] == CHECKPOINT_ID and value == STOP_OPTION: person["step"] = len(QUESTIONS); person["finished"] = datetime.now().isoformat(timespec="seconds"); self.save(); self.export_csv(); return prefix + DONE_SHORT
        person["reading"] = False; after = question.get("after")
        if after:
            said = after(person["answers"])
            if said: person["reading"] = True; prefix += said + "\n\n" + "—" * 20 + "\n\n"
        person["step"] = self._next(step + 1, person["answers"])
        if person["step"] >= len(QUESTIONS): person["finished"] = datetime.now().isoformat(timespec="seconds"); self.save(); self.export_csv(); return prefix + DONE_FULL + "\n\n" + self.summary(user_id)
    ...
    def _ask(self, user_id: str, step: int, *, hint: bool = True) -> str:
        ...
        if question["kind"] == "choice" and self.list_options:
            parts.append(""); parts.extend(f"{n}. {name}" for n, name in enumerate(question["options"], 1)); parts.append(""); parts.append("Напишите номера через запятую, например: 1, 3" if question.get("multi") else "Напишите номер ответа.")
    ...
    def current(self, user_id: str) -> tuple[int, dict[str, Any]] | None:
        ...
        step = self._next(person.get("step", 0), person.get("answers", {})); return None if step >= len(QUESTIONS) else (step, QUESTIONS[step])
    @staticmethod
    ...
    def picked(self, user_id: str, step: int) -> list[int]:
        pending = (self.state.get(user_id) or {}).get("pending") or {}; return list(pending.get("picked", [])) if pending.get("step") == step else []
    ...
    def toggle(self, user_id: str, step: int, index: int) -> bool:
        ...
        else: picked = [i for i in picked if not self._is_none_option(options[i])]; picked.append(index)
    ...
    def _check(self, question: dict[str, Any], text: str) -> tuple[bool, str, str]:
        kind = question["kind"]
        if kind == "phone":
            digits = re.sub(r"\D", "", text); return (True, text, "") if len(digits) >= 10 else (False, "", "Не похоже на номер телефона — в нём должно быть не меньше десяти цифр.")
        if kind == "address": return (False, "", "В адресе нужен номер дома — без него координатор не найдёт. Напишите ещё раз, пожалуйста.") if len(text) < 6 or not re.search(r"\d", text) else (True, text, "")
        if kind == "email":
        ...
        if kind == "choice": return self._check_multi(question["options"], text) if question.get("multi") else self._check_one(question["options"], text)
    ...
    def _check_one(options: list[str], text: str) -> tuple[bool, str, str]:
        if text.isdigit():
            number = int(text); return (True, options[number - 1], "") if 1 <= number <= len(options) else (False, "", f"Нужен номер от 1 до {len(options)}.")
        for name in options:
    ...
    def summary(self, user_id: str) -> str:
        ...
        if person.get("alerts"): lines.append(""); lines.append("Отмечено как требующее внимания:"); lines.extend(f"  — {note}" for note in person["alerts"])
        return "\n".join(lines)

    def brief(self, user_id: str) -> str:
        person = self.state.get(user_id)
        if not person: return ""
        a = person["answers"]; куда = a.get("address") or a.get("district", ""); bits = [a.get("patient_name") or a.get("name", "без имени"), a.get("phone", "телефон не указан"), куда, a.get("mobility") or a.get("need", "")]; line = " · ".join(b for b in bits if b)
    ...
    def stats(self) -> tuple[int, int]:
        return len(self.state), sum(1 for p in self.state.values() if p.get("finished"))
